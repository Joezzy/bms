...
class _WIPScreenState extends State<WIPScreen> {
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child:   Text("WIP",style: TextStyle(fontSize: MySize.size16,fontWeight: FontWeight.bold),),
              ),
              ...
              Container(
                  child: ListView.builder(
                      ...
                      itemCount: wipList.length,
                      itemBuilder: (BuildContext context, index) {
                        if (index == wipList.length) {
                          return Container();
                        }
                        {
                          Wip result = wipList[index];
                          return Column(
                            children: [
                              ListTile(
                                  title:Row(
                                    ...
                                    children: [
                                      Text("${result.orderDetails!.orderNumber}"),
                                      Text("${result.status}",style: TextStyle(fontWeight: FontWeight.w500,fontSize: 12),),
                                    ],
                                  ),

                                  subtitle: Text( " ${ DateFormat('dd-MM-yyyy').format(DateTime.parse(result.dateCreated.toString()))} , ${ DateFormat('hh:mm a').format(DateTime.parse(result.dateCreated.toString()))}"),
                                  onTap: (){
                                    Navigator.of(context,rootNavigator: true).push(
                                      MaterialPageRoute(
                                        builder: (context) => WIPDetailScreen(
                                          wip: result,
                                        ),
                                      ),
                                    );
  ...
  void getWip() async {
    ...
    String url = baseUrl + '/wip/?business_id=${businessProvider.baseStore}';
    ...
    if (res[0]["status"] == "failed") {
      ...
      setState(() {
        wipList= wipFromJson(json.encode(myData));
        isLoading=false;
      });
