...
import 'package:user/screens/dashboard/operation/orderDetailScreen.dart';
...
class _OrderScreenState extends State<OrderScreen> {
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child:   Text("Orders",style: TextStyle(fontSize: MySize.size16,fontWeight: FontWeight.bold),),
              ),
                              ...
                              ListTile(
                                  title:Row(
                                    ...
                                    children: [
                                      ...
                                      Text("${result.orderId}",style: TextStyle(fontWeight: FontWeight.w500,fontSize: 12),),
                                    ],
                                  ),

                                  subtitle: Text("${AppTheme.money(double.parse(result.unitCost.toString()))}x ${result.qty} =  ${AppTheme.money(double.parse(result.totalCost.toString()))}"),
                                  onTap: (){
  ...
  void getOrders() async {
    ...
    String url = baseUrl + '/orders/?business_id=${businessProvider.baseStore}';
    if (mounted)
      setState(() {
        isLoading = true;
      });
    ...
    if (res[0]["status"] == "failed") {
      ...
      setState(() {
        orderList= orderFromJson(json.encode(myData));
        isLoading=false;
      });
